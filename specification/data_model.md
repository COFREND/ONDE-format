...
The storage of each class field depends on its type and the storage as defined in the class specification. All fields that reference ONDE objects or contain arrays of such references can be stored as either HDF5 groups containing the actual object(s) or as HDF5 datasets. For single references, the HDF5 group would contain the object itself or the HDF5 dataset would be a 0-dimensional array containing a single reference. The HDF5 group structure to accommodate arrays of references is discussed below. Arrays of references can alternatively be stored using an HDF5 dataset if the objects are located elsewhere.

For each non-class field, the specification defines the storage as either attribute or dataset. If the storage is specified as attribute then the field is normally stored as an HDF5 attribute. If the storage is specified as dataset, the field contents should be an array of data and the field is instead stored as an HDF5 dataset. Attributes will typically be used for fields that have a small size (scalar, string or small arrays), while datasets will be used for  arrays with a significant size. The specification also defines whether a field is optional or mandatory, as well as the constraints on their dimensions and the range of accessible values (minimum and maximum for scalar values or allowed values for some strings).

Storing fields as HDF5 groups allows a hierarchical layout of the ONDE file where objects are contained within other objects. Such containment confers no special status within the ONDE data model and is conceptually no different from any other reference to the object. In general, an object shouldn't be duplicated for the purpose of placing it within another object. Rather, the object should be placed in a single location and should be referenced when needed elsewhere.

HDF5 groups can indeed be used to store arrays of objects or object references, even though HDF5 groups do not support array access. In this case, the HDF5 group should contain an attribute named "ONDE:SHAPE" with an integer array defining the dimensions of the field. The elements of the array are stored in precisely named HDF5 datasets or subgroups, according to whether they are references or entire objects. The dataset or subgroup names are the array indexes, starting with zero, with each index at least five digits but otherwise as short as possible, separated with commas and with surrounding parentheses, for example "(00001,00005)".
