...
A field that contains an array of objects or object references and is specified as attribute/group storage may indeed be stored in the form of an HDF5 group, even though HDF5 groups do not support array access. In this case, the HDF5 group should contain an attribute named "ONDE:SHAPE" with an integer array defining the dimensions of the field. The elements of the array are stored in precisely named HDF5 attributes or subgroups, according to whether they are references or entire objects. The attribute or subgroup names are the array indexes, starting with zero, with each index at least five digits but otherwise as short as possible, separated with commas and with surrounding parentheses, for example "(00001,00005)".
